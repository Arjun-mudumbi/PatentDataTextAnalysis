...
#    print(patents)
    ...
    d=patents.docs
    p=[d[i] for i in range(len(d))]#limit=21; start=1; step=1;
...
# Topic 6 - thanks
#a = args.file[1]
testadd=address.iloc[1]
#%%%%5
lda_model = LdaModel(corpus,num_topics=3)														# to load lda model
...
#%%%%%%%%%%%%%%555
diction =dict(dictionary)
#%%%%%%%%%%%%%55
from sklearn.decomposition import LatentDirichletAllocation
lda=LatentDirichletAllocation(n_components=10)
lda.fit_transform(text)
#%%%%%%%%%%55
text2=""
text2=text2+str([str(p.ab_en) for i in range(len(p))])

#%%%%%%%%%%
text2=np.ndarray(text2)
#%%%%%

text2=text2.reshape(-1,1)

text2=pd.DataFrame(text2)
lda.fit_transform(text2)